...
const ratingLabel = (rating) => rating?.rating_status === 'AVAILABLE' ? `${rating.bayesian_rating} / 5` : rating?.rating_status === 'INSUFFICIENT_PRIOR_SAMPLE' ? 'Rating developing' : 'No reviews yet';
...
const HandymanDashboardPage = () => {
  ...
  return <main className="participant-overview">
    <header><div><p className="eyebrow">Handyman workspace</p><h1>Welcome back, {account.full_name || 'Handyman'}</h1><p>Review assigned Jobs, bids and the work that needs attention.</p></div><button onClick={() => load(true)} disabled={state.refreshing}>{state.refreshing ? 'Refreshing…' : 'Refresh'}</button></header>
    ...
    <section className="participant-overview__metrics" aria-label="Account summary">
      ...
      <article><span>Rating</span><strong>{ratingLabel(data.rating_summary)}</strong><span>{data.rating_summary.review_count} reviews</span></article>
    </section>
    <div className="participant-overview__columns">
      ...
      <section><div className="section-heading"><h2>Recent assigned Jobs</h2><Link to="/handyman/my-jobs">View all</Link></div>{data.recent_jobs.length ? <div className="overview-list">{data.recent_jobs.map((item) => <Link to={item.destination} key={item.job_id}><div><strong>{item.service?.name || 'Service Job'}</strong><span>{item.status}{item.review_state === 'PENDING' ? ' · Review pending' : ''}</span></div><time>{new Date(item.updated_at).toLocaleDateString()}</time></Link>)}</div> : <div className="empty-copy"><p>No assigned Jobs yet.</p><Link to="/handyman/find-jobs">Browse available Jobs</Link></div>}</section>
    </div>
  </main>;
};
