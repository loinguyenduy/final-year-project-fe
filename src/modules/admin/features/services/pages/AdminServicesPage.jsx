...
const blankForm = { service_code: '', name: '', is_active: true };
...
const AdminServicesPage = () => {
  ...
  const openCreate = () => { setEditor({ mode: 'create', service: null }); setForm(blankForm); };
  const openEdit = (service) => {
    setEditor({ mode: 'edit', service });
    setForm({ service_code: service.service_code, name: service.name, is_active: service.is_active });
  };
  const closeEditor = () => { if (!submitting) { setEditor(null); setForm(blankForm); } };
  const save = async (event) => {
    event.preventDefault(); setSubmitting(true);
    try {
      if (editor?.mode === 'edit') await updateAdminService(editor.service.id, { name: form.name });
      else await createAdminService({ service_code: form.service_code, name: form.name, is_active: form.is_active });
      toast.success(`Service ${editor?.mode === 'edit' ? 'updated' : 'created'} successfully.`);
      setEditor(null); setForm(blankForm); await load();
  ...
  return <div className="admin-services-page">
    <PageHeader title="Services" description="Manage the canonical service catalogue. Existing Jobs remain linked when a Service is inactive." />
    <div className="service-toolbar">
      <button type="button" className="service-primary" onClick={openCreate}><FaPlus /> Create service</button>
    </div>
    ...
    {loading ? <LoadingState message="Loading Services..." /> : error ? <ErrorState message={error} onRetry={() => load()} /> : !data.items.length ? <EmptyState icon={<FaTools />} message="No Services match these filters." /> : <>
      <div className="service-grid">{data.items.map((service) => <article className="service-item" key={service.id}>
        ...
        <div className="service-actions"><button type="button" onClick={() => openEdit(service)}>Edit</button><button type="button" className={service.is_active ? 'danger' : ''} onClick={() => setStatusTarget(service)}>{service.is_active ? 'Deactivate' : 'Activate'}</button></div>
      </article>)}</div>
    ...
    <AdminModal open={editorOpen} titleId="service-editor-title" submitting={submitting} onClose={closeEditor} className="service-modal"><form onSubmit={save}>
      <header><div><span>Service catalogue</span><h2 id="service-editor-title">{isEditing ? 'Edit service' : 'Create service'}</h2></div><button type="button" aria-label="Close" onClick={closeEditor}>×</button></header>
      <div className="service-modal-body">
        <label>Service code<input value={form.service_code} disabled={isEditing} required minLength={2} maxLength={50} pattern="[A-Z0-9_]+" onChange={(event) => setForm({ ...form, service_code: event.target.value.toUpperCase() })} /><small>Uppercase letters, digits and underscores. This value is immutable.</small></label>
        ...
        {!isEditing && <label className="service-checkbox"><input type="checkbox" checked={form.is_active} onChange={(event) => setForm({ ...form, is_active: event.target.checked })} /> Active immediately</label>}
