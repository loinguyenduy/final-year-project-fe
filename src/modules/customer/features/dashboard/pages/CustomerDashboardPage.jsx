...
import { getParticipantStatusLabel } from '../../../../identity/utils/participantDisplay';
...
const CustomerDashboardPage = () => {
  ...
  return <main className="participant-overview">
    <header><div><p className="eyebrow">Customer workspace</p><h1>Welcome back, {account.full_name || 'Customer'}</h1><p>Track the Jobs and actions that need your attention.{state.refreshing ? ' Refreshing…' : ''}</p></div></header>
    ...
    <section className="participant-overview__metrics" aria-label="Account summary">
      ...
      <article><span>Completed Jobs</span><strong>{data.job_summary.closed}</strong><Link to="/customer/my-jobs?view=CLOSED">View history</Link></article>
    ...
    <div className="participant-overview__columns">
      ...
      <section><div className="section-heading"><h2>Recent Jobs</h2><Link to="/customer/my-jobs">View all</Link></div>{data.recent_jobs.length ? <div className="overview-list">{data.recent_jobs.map((item) => <Link to={item.destination} key={item.job_id}><div><strong>{item.service?.name || 'Service Job'}</strong><span>{getParticipantStatusLabel(item.status)}{item.review_state === 'PENDING' ? ' · Review pending' : ''}</span></div><time>{new Date(item.updated_at).toLocaleDateString()}</time></Link>)}</div> : <div className="empty-copy"><p>No Jobs yet.</p><Link to="/customer/ai-diagnosis">Post your first Job</Link></div>}</section>
    </div>
  </main>;
};
